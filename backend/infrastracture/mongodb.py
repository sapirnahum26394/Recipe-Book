...
class MongoDatabase:
    ...
    def update_one(self, collection_name, filter, update, upsert=False):
        return self.get_collection(collection_name).update_one(filter, update, upsert=upsert)
    ...
    def replace_one(self, collection_name, filter, update, upsert=False):
        return self.get_collection(collection_name).replace_one(filter, update, upsert=upsert)
